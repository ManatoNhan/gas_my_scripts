 ...
 * @param {Object} invoiceData - 請求書ログデータ
 * @return {Object} MoneyForward API形式のデータ
 */
function convertToMFInvoiceFormat(invoiceData) {
  try {
    // スプレッドシートデータ取得
    const data = getSpreadsheetDataFromUrl(invoiceData.outputUrl);
    ...
    // 取引先ID取得（商談情報から）
    const partnerId = invoiceData.partnerId || getPartnerIdFromAccount(invoiceData.accountId, invoiceData.accountName);
    ...
    const mfData = {
      ...
      title: companyInfo[3] || invoiceData.subject || '請求書',
      billing_date: formatDate(companyInfo[4]) || formatDate(new Date()),
      due_date: formatDate(companyInfo[5]) || '',
 ...
 */
function validateMFInvoiceData(mfData) {
  const errors = [];
  ...
  if (!mfData.billing_date) {
    errors.push('請求日が設定されていません');
  }
  
  if (!mfData.due_date) {
    errors.push('支払期限が設定されていません');
  }
 ...
 */
function createInvoiceWithPartner(mfData) {
  try {
    const mfApi = new MoneyForwardAPI();
    const service = mfApi.getOAuthService();
    ...
    const response = UrlFetchApp.fetch(`${mfApi.apiBaseUrl}/invoice_template_billings`, {
      method: 'POST',
      headers: {
        'Authorization': `Bearer ${service.getAccessToken()}`,
        'Content-Type': 'application/json'
    ...
    if (response.getResponseCode() === 201 || response.getResponseCode() === 200) {
      return {
        success: true,
        invoiceNumber: result.billing_number,
        invoiceId: result.id,
        error: null
      };
    } else {
...
/**
 * Salesforceの商談に請求書番号を更新
 * @param {string} opportunityId - 商談ID
 * @param {string} invoiceId - MoneyForward請求書ID
 */
function updateSalesforceInvoiceId(opportunityId, invoiceId) {
  try {
    const accessToken = getSalesforceAccessToken();
    const instanceUrl = PropertiesService.getScriptProperties().getProperty('SF_INSTANCE_URL');
    ...
    console.error('Salesforce更新エラー:', error);
    throw error;
  }
}
