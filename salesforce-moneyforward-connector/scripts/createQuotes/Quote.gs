 ...
 * 見積書発行ダイアログを表示
 */
function showQuotePublishDialog() {
    ...
    <div style="font-family: Arial, sans-serif; padding: 20px;">
      ...
      <div id="status" style="margin: 20px 0;">
        ...
        <div style="width: 100%; background-color: #f0f0f0; border-radius: 5px;">
          <div id="progressBar" style="width: 0%; height: 20px; background-color: #2196F3; border-radius: 5px; transition: width 0.3s;"></div>
        </div>
        <p id="progressText" style="font-size: 12px; color: #666; margin-top: 5px;">0 / 0</p>
      </div>
      
      <div style="margin-top: 20px;">
        <button onclick="startPublish()" style="padding: 10px 20px; background-color: #2196F3; color: white; border: none; border-radius: 5px; cursor: pointer;">
          発行開始
        </button>
    ...
    <script>
      function startPublish() {
        ...
        google.script.run
          ...
          .publishSelectedQuotesWithProgress();
      }
  ...
  SpreadsheetApp.getUi().showModalDialog(html, '見積書発行');
}

function publishSelectedQuotesWithProgress() {
  ...
  try {
    ...
    for (let i = 0; i < targetRows.length; i++) {
      ...
      try {
        ...
        const quoteData = getInvoiceDataFromLog(sheet, rowIndex);
        
        if (!quoteData.outputUrl) {
          throw new Error('アウトプットURLが設定されていません');
        }
        
        // 商談情報を取得
        const opportunity = getOpportunityInfo(quoteData.opportunityId);
        
        // quoteDataに必要な情報を追加
        quoteData.accountId = opportunity.AccountId;
        quoteData.accountName = opportunity.Account.Name;
        quoteData.partnerId = opportunity.Account.MoneyForward_Partner_ID__c;
        
        if (!quoteData.partnerId) {
          throw new Error(`取引先が同期されていません: ${quoteData.accountName}`);
        }
        
        // データ変換
        const mfData = convertToMFQuoteFormat(quoteData);
        
        const result = createQuoteWithPartner(mfData);
        
        if (result.success) {
          // updateSalesforcequoteId(quoteData.opportunityId, result.quoteId);
          updateLogStatus(sheet, rowIndex, '完了', result.quoteNumber);
          successCount++;
        } else {
 ...
 * 未処理のみ選択
 */
function selectUnprocessedquotes() {
  const sheet = SpreadsheetApp.getActiveSpreadsheet().getSheetByName('見積書ログ');
  const lastRow = sheet.getLastRow();
